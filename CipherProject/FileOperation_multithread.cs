...
{
    /// <summary>
    /// класс для запоминания/установки дат файла (создания, доступа, записи)
    /// </summary>
    class DatesInFileOption
    {
        private DateTime fileDateCreation;
        private DateTime fileDateLastModify;
        private DateTime fileDateLastAccess;
        public DatesInFileOption()
        {
    ...
    class FileOperation
    {
        ...
        private void EncryptFileRijndael(string inFileFullName, string hash, string inFile)
        {
            ...
            DatesInFileOption datesOfFile = new DatesInFileOption();
            datesOfFile.GetFileDates(inFileFullName);
            ...
            {
                if ((progress == 0) && (addRandomBlock == 1))
                {
            ...
            {
                datesOfFile.SetFileDates(outFile);
            }
        }
        ...
        public void EncryptListFiles(List<string> selectedFiles, string sPass, int cryptoAlg)
        {
            if (selectedFiles.Count > 0)
            {
                ...
                if (h.Length > 50)//есть пароль
                {
                    ...
                    po.MaxDegreeOfParallelism = 2;// Environment.ProcessorCount;
                    Parallel.ForEach<string>(selectedFiles, po, (inFile, pls) =>
                    {
                        inFileFullName = pathFrom + inFile;
                        if (System.IO.File.Exists(inFileFullName))
                        {
